...
export default function Dashboard({ onLogout, onSelectAssessment }: DashboardProps) {
  ...
  const fetchProviderAndCountry = async () => {
    const { data: { user } } = await supabase.auth.getUser()
    if (user) {
      const { data, error } = await supabase
        .from('providers')
        .select('id, email, full_name, license_number, country, created_at')
        .eq('user_id', user.id)
        .maybeSingle()
  ...
  const fetchAssessmentsWithCountry = async (tab: 'pending' | 'reviewed', country: string, forceRefresh = false) => {
    ...
    try {
      ...
      if (!initialLoadComplete) {
        // Don't clear main loading here, it's handled in initializeDashboard
      } else {
