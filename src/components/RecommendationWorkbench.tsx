...
if (!webhookResponse.ok) {
  throw new Error(`Edge Function failed with status: ${webhookResponse.status}`);
}
...
console.log('✅ Supabase Edge Function response:', result);
      ...
      <div className="mb-8">
        ...
        {isReadOnly ? (
          ...
          <select
            ...
            className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent font-medium text-gray-900 h-12 bg-white shadow-sm"
          >
  ...
  {recommendations.map((recommendation) => (
    <div key={recommendation.category} className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-4 border border-gray-100">
      <label className="block text-sm font-semibold text-blue-600 mb-2 uppercase tracking-wide">
        {recommendation.category}
      </label>
      {isReadOnly ? (
        <div className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg font-medium text-gray-900 text-sm whitespace-pre-wrap shadow-sm">
        ...
        <textarea
          ...
          rows={2}
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-blue-500 focus:border-transparent resize-none font-medium text-gray-900 bg-white text-sm shadow-sm"
      ...
      <div className="mb-8">
        ...
        {isReadOnly ? (
          <div className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl font-medium text-gray-900 min-h-[100px] whitespace-pre-wrap shadow-sm">
          ...
          <textarea
            ...
            rows={4}
            className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none font-medium text-gray-900 bg-white shadow-sm"
      ...
      {!isReadOnly && (
        <div className="flex space-x-4">
          <button
            ...
            className="flex-1 px-6 py-3 border-2 border-gray-300 rounded-xl text-gray-900 hover:bg-gray-50 hover:border-gray-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-semibold flex items-center justify-center h-12 bg-white shadow-sm"
          >
            {saving ? (
              <>
          ...
          <button
            ...
            disabled={saving || approving}
            className="flex-1 px-6 py-3 bg-gradient-to-r from-blue-500 to-teal-500 text-white rounded-xl hover:from-blue-600 hover:to-teal-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-semibold flex items-center justify-center h-12 shadow-lg hover:shadow-xl"
          >
            {approving ? (
              <>
                <Loader2 className="w-4 h-4 mr-2 animate-spin" />
